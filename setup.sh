...
setup_azh() {
    ...
    #   - "AZH_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "AZH_" variables is documented below.
    #
    ...
    if [ "${AZH_SETUP}" = "1" ]; then
        >&2 echo "the AZH analysis was already succesfully setup"
    ...
    #
    # global variables
    # (AZH = AZH, CF = columnflow)
    #

    # start exporting variables
    export AZH_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${AZH_BASE}"
    export CF_REPO_BASE_ALIAS="AZH_BASE"
    ...
    if [ "${CF_REMOTE_ENV}" != "1" ]; then
        ...
        cf_setup_interactive "${CF_SETUP_NAME}" "${AZH_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${AZH_BASE}/bin:${PATH}"
    export PYTHONPATH="${AZH_BASE}:${AZH_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${AZH_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${AZH_BASE}/modules" ); do
            cf_init_submodule "${AZH_BASE}" "modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${LAW_HOME:-${AZH_BASE}/.law}"
    export LAW_CONFIG_FILE="${LAW_CONFIG_FILE:-${AZH_BASE}/law.cfg}"
...
main() {
    ...
    if setup_azh "$@"; then
        cf_color green "AZH analysis successfully setup"
        return "0"
    else
        local code="$?"
